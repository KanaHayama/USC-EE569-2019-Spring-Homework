...
def window_process3(samples, kernel_size, stride):
    ...
    idx1 = range(0, h-kernel_size+1, stride)
    print(idx1)
...
def multi_Saab_transform(images, labels, kernel_sizes, num_kernels, energy_percent, use_num_images, use_classes, overlapping=True, maxpooling=True):
    '''
    Do the PCA "training".
    ...
    for i in range(num_layers):
        ...
        window_stride = 1 if overlapping else kernel_sizes[i]
        sample_patches = window_process2(sample_images, kernel_sizes[i], window_stride)
...
# Initialize
def initialize(sample_images, pca_params, overlapping=True, maxpooling=True):
    ...
    for i in range(num_layers):
        ...
        if i==0:
            ...
            sample_patches = window_process3(sample_images, kernel_sizes[i], window_stride)
            print('sample_patches1: {}'.format(sample_patches.shape))
